...
In Pi, replace all `Task` tool references with the `subagent` tool. The subagent tool supports three modes:
...
**Parallel mode** (for independent tasks that do not touch the same files):
```
subagent({
  tasks: [
...
The spec-reviewer and code-quality-reviewer prompt templates (`./spec-reviewer-prompt.md`, `./code-quality-reviewer-prompt.md`) remain the same. Pass their content as part of the `task` string when dispatching reviewer subagents.
