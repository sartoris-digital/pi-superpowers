...
**Plan review dispatch:** Use the subagent tool for plan document review:
```
subagent({ agent: "code-reviewer", task: "Review plan document at <path>. Compare against spec at <spec-path>. Check for completeness, correct task ordering, and TDD compliance." })
```

**Execution handoff:** Use `superpowers:subagent-driven-development` with the `subagent` tool for implementation. See the Pi Platform Notes in that skill for dispatch syntax.
