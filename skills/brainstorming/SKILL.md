...
- The visual companion feature (local HTTP server for mockups) is not available in Pi. For visual design work, describe the design in text or use external tools.
- Spec review uses the subagent tool:
  ```
  subagent({ agent: "code-reviewer", task: "Review spec document at <path>. Check for completeness, consistency, and implementability." })
  ```
